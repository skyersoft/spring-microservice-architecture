...
import com.skyersoft.fxdata.config.KafkaProperties;
...
class FxdataKafkaResourceIT {
    ...
    void setup() {
        ...
        FxdataKafkaResource kafkaResource = new FxdataKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/fxdata-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/fxdata-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
